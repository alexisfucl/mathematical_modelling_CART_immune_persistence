...
%
% Figure 3 details (caption):
% (a) k2 = 1.5  × 10^-10
% (b) k2 = 2.5 × 10^-10
% (c) k2 = 4.0  × 10^-10
% (d) P(elimination) vs k2, based on 10^3 realisations.
%
% References in our PDF:
% - Model equations (2.1)–(2.3)
% - Table 1 baseline parameters
% - Figure 3 caption + MRD/hybrid thresholds described in text
...
dt      = 0.1;     % hybrid ODE increment (paper uses delta t = 0.1) 
Lambda  = 1e3;     % switch threshold (paper uses Lambda = 10^3)
MRD     = 1e6;     % MRD detection threshold (paper uses 10^6)
ymin    = 1e3;
ymax    = 1e12;
...
% =========================
% x-axis in the figure is labeled in units of 1e-10, from ~1.4 to 2
k2_grid = linspace(3, 4.5, 2) * 1e-10;

nReps   = 10^1;   % as in caption
...
yticklabels(ax1, {'10^{3}','10^{6}','10^{9}','10^{12}'});  % optional; MATLAB often formats fine on its own
ylim(ax1,[1e3 1e12]);
% Increase linewidth for all line objects in this axes
lines = findall(ax1, 'Type', 'Line');
set(lines, 'LineWidth', 2);  % <-- adjust this value to desired line width
...
yticklabels(ax2, {'10^{3}','10^{6}','10^{9}','10^{12}'}); % optional; MATLAB often formats fine on its own
ylim(ax2,[1e3 1e12]);
% Increase linewidth for all line objects in this axes
...
% Use TeX interpreter for tick labels so superscripts render correctly (e.g. 10^{3})
set(ax3, 'TickLabelInterpreter', 'tex');
yticklabels(ax3, {'10^{3}','10^{6}','10^{9}','10^{12}'});  % ensure superscripts render as superscripts
ylim(ax3,[1e3 1e12]);
% Increase linewidth for all line objects in this axes
...
% xlabel(ax4, 'CAR T induced tumour death rate  \times 10^{-10}');
% ylabel(ax4, 'Elimination probability');
% xlim(ax4, [3 4.5]);
% ylim(ax4, [0 1]);
% grid(ax4,'on');
% % Make outer axes box grey while keeping tick labels and axis labels black
% set(ax4, 'Box','on');
% title(ax4, '(d)','FontWeight','normal', 'Color',[0 0 0]);
...
% % Also save in MATLAB figure format (.fig) so the figure can be reopened and edited in MATLAB
...
%
% From Table 1 / parameter estimation section:
% k1 = 0.3 day^-1
% k2 = set per panel
% k3 = 1e-9 day^-1 cell^-1
% k4 = 0.3 day^-1
% gamma = 0.3 day^-1
% delta = 0.2 day^-1
% epsilon = 0.01 day^-1
% K = 1e12
% N = 6
% B1/2 (phenotypic switching threshold): table line shows 10^9 (range 10^8–10^10)
P.k1     = 0.2;
...
P.Bhalf  = 1e9;  % B_{1/2}. (If you later confirm you used 1e8 in plots, change here.)
...
P.CAR0   = 4.1e8;
P.M0     = .468 * P.CAR0;
P.A0     = 0;                    % not specified as nonzero in text
P.E0each = (.532 * P.CAR0)/P.N; % equally split

% Initial blast burden:
% The paper text ties 100% disease burden to 1e12; your earlier drafts use 2e11.
% Figure 3 starts below carrying capacity; 2e11 is consistent with that shape.
P.B0     = 2e11;
...
% As written: dA = k3 M B - k4 A*(1-frac) - k4 A*frac = k3 M B - k4 A
...
% plot(sim.t, sim.A, 'k:', 'LineWidth', 1.3);
...
% Annotations ("MRD positive/negative") roughly where they appear in the figure
