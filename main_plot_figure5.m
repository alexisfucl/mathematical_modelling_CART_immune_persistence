...
while t < tEnd - timeTol
    ...
    % A species is fully stochastic only if no currently deterministic
    % reaction changes it. When this first occurs, use unbiased stochastic
    % rounding so the state becomes integer-valued without changing its
    % expectation. Products of deterministic reactions are therefore NOT
    % rounded merely because their population is small.
    % -------------------------------------------------------------
...
% A stochastic reaction may remove one cell from a species that is also
% receiving a deterministic flux and is therefore temporarily fractional.
% Such a jump is permitted only when the available population is at least
% the amount removed by the stoichiometric update. This prevents negative
% populations without rounding products of deterministic reactions.
...
% R4       : memory activation
% R5       : activated cell -> memory lineage
% R6       : activated cell -> effector lineage
% R6+i     : Ei -> 2 E(i+1), i=1,...,N-1
% R(N+6)   : terminal effector death
% R(N+7)   : memory death
% =====================================================================
